...
describe('MetricsInterceptor (company-service)', () => {
  ...
  beforeEach(() => {
    metricsService = {
      getServiceName: jest.fn().mockReturnValue('company-service'),
  ...
  it('registra métricas completas en una request exitosa', async () => {
    const request = {
      method: 'GET',
      route: { path: '/company/v1/companies/:id/members' },
    };
    ...
    expect(endTimer).toHaveBeenCalledWith({ status: '200', service: 'company-service' });
  });
  ...
  it('usa url si route y baseUrl no están disponibles', async () => {
    ...
    expect(metricsService.httpRequestCounter.inc).toHaveBeenCalledWith(
      expect.objectContaining({ route: '/companies' }),
    );
  });
