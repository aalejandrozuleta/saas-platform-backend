...
const PERMISSION_META: Record<Permission, { description: string; module: string }> = {
  // Plataforma
  [Permission.PLATFORM_MAINTENANCE]:   { description: 'Activar/desactivar mantenimiento de la plataforma', module: 'platform' },
  [Permission.PLATFORM_FEATURE_FLAGS]: { description: 'Gestionar feature flags de la plataforma',          module: 'platform' },
  [Permission.PLATFORM_TENANTS]:       { description: 'Gestionar tenants/organizaciones en la plataforma', module: 'platform' },
  // Organización
  [Permission.ORG_MANAGE]:             { description: 'Gestionar configuración de la organización',         module: 'org' },
  ...
  [Permission.EMPLOYEE_MANAGE_PERMISSIONS]: { description: 'Modificar permisos de empleados de su organización', module: 'employee' },
  [Permission.EMPLOYEE_READ]:               { description: 'Ver detalle de un empleado',                         module: 'employee' },
  [Permission.EMPLOYEE_LIST]:               { description: 'Listar empleados de la organización',                module: 'employee' },
  // Facturas
  ...
  [Permission.INVOICE_READ_OWN]: { description: 'Ver únicamente las facturas propias',        module: 'invoice' },
  // Ventas
  ...
  [Permission.FINANCE_MANAGE]: { description: 'Gestionar configuración financiera',module: 'finance' },
  // Reportes
  ...
  for (const [role, perms] of Object.entries(ROLE_PERMISSIONS)) {
    for (const permCode of perms) {
      await prisma.rolePermission.upsert({
        where:  { role_permissionCode: { role: role as any, permissionCode: permCode } },
        update: {},
        create: { role: role as any, permissionCode: permCode },
      });
    }
