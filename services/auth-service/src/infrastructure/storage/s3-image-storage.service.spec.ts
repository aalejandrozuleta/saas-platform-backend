...
describe('S3ImageStorageService', () => {
  ...
  describe('upload', () => {
    it('debe subir el objeto y devolver la URL pública', async () => {
      const url = await service.upload('avatars/user-1.png', Buffer.from('img'), 'image/png');

      expect(sendMock).toHaveBeenCalledTimes(1);
      expect(url).toBe('http://localhost:9000/user-avatars/avatars/user-1.png');
    });
  });
});
