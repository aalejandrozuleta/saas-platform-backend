...
export class LoginUserUseCase {
  ...
  async execute(
    ...
    if (securityProfile?.twoFactorEnabled) {
      this.throwChallenge(user, securityProfile, context, LoginChallengeReason.TWO_FACTOR_REQUIRED);
    }
  ...
  private async resolveLoginDevice(
    ...
    if (!device) {
      this.throwChallenge(user, securityProfile, context, LoginChallengeReason.NEW_DEVICE);
    }

    if (!device.isTrusted) {
      this.throwChallenge(user, securityProfile, context, LoginChallengeReason.UNTRUSTED_DEVICE);
    }
    ...
    if (
      ...
      this.throwChallenge(user, securityProfile, context, LoginChallengeReason.UNTRUSTED_COUNTRY);
    }
  ...
  /**
   * Genera un challenge token de corta duración y lanza el error de dominio
   * `SECURITY_CHALLENGE_REQUIRED` correspondiente. Usado tanto por los
   * chequeos de dispositivo/país no confiable como por el requerimiento de
   * 2FA en cada login.
   */
  private throwChallenge(
    user: User,
    securityProfile: LoginSecurityProfile | null,
    context: LoginContext,
