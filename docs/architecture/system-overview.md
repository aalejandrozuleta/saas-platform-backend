...
- **company-service** — tenant (`Company`) y membresía de trabajadores (`CompanyMembership`); referencia usuarios de auth-service, nunca los crea.
- **shared** — librería común (logger, excepciones, validación, Swagger, métricas, contexto async).

> Los servicios `order-service` y `payment-service` mencionados en versiones anteriores de este documento **no están implementados**. Forman parte del roadmap a futuro — ver [README-SaaS-Platform.md](../../README-SaaS-Platform.md) (Fase 4 en adelante).
